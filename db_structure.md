...
| company_id | int | NO | FK | NO | 소속 회사 코드 (tb_company -> id) |
...
-- tb_celeb 샘플 데이터
INSERT INTO tb_celeb (user_id, company_id, celeb_type) VALUES
(2, 1, '가수');
...
| --- | --- | --- | --- | --- | --- |
| id | int | NO | PK | NO | Auto Increment |
| post_id | int | NO | FK | NO | 게시글 ID (tb_post -> id) |
| writer_id | int | NO | FK | NO | 작성자 ID (tb_user -> user_id) |
| content | TEXT | NO | - | NO | 댓글 내용 |
| created_at | timestamp | NO | - | NO | 작성일시 (CURRENT_TIMESTAMP) |
| visible | boolean | NO | - | NO | 삭제 여부 (삭제 시 FALSE) |
...
-- tb_comment 샘플 데이터
INSERT INTO tb_comment (post_id, writer_id, content) VALUES
(1, 1, '첫 번째 게시글에 대한 댓글입니다.'),
(1, 2, '셀럽의 댓글입니다.'),
(2, 1, '공지사항에 대한 팬의 댓글입니다.');
```
