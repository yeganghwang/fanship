...
- **회원가입**: 성공적으로 동작하며, 중복 데이터에 대한 예외 처리도 잘 작동합니다. `position` 필드 및 `celeb` 유형 사용자에 대한 추가 유효성 검사가 정상적으로 작동합니다. 비밀번호 해싱 실패에 대한 오류 처리를 추가했습니다.
    ...
    - **게시글 수정**: JWT 인증 기반으로 게시글 수정이 성공적으로 동작합니다. 작성자만 수정할 수 있도록 권한 검사가 작동합니다.
    - **굿즈 수정**: JWT 인증 기반으로 굿즈 수정이 성공적으로 동작합니다. 판매자만 수정할 수 있도록 권한 검사가 작동합니다.
    - **게시글 삭제**: JWT 인증 기반으로 게시글 삭제가 성공적으로 동작합니다. 작성자만 삭제할 수 있도록 권한 검사가 작동합니다. (소프트 삭제)
    - **굿즈 삭제**: JWT 인증 기반으로 굿즈 삭제가 성공적으로 동작합니다. 판매자만 삭제할 수 있도록 권한 검사가 작동합니다. (소프트 삭제)
    - **사용자가 작성한 게시글 목록 조회**: JWT 인증 기반으로 특정 사용자가 작성한 게시글 목록을 성공적으로 조회합니다.
    - **사용자가 등록한 굿즈 목록 조회**: JWT 인증 기반으로 특정 사용자가 등록한 굿즈 목록을 성공적으로 조회합니다.
